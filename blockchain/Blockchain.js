...
class Blockchain {
    ...
    proofOfWork() {
        ...
        let nonce = 0;
        let prevHash = this.chain.length !== 0 ? this.chain[this.chain.length - 1].hash : '0';
        let hash = this.getHash(prevHash, this.pendingTransactions, nonce.toString());
        while (hash.slice(0, difficulty) !== "0".repeat(difficulty)) {
            nonce += 1;
            hash = this.getHash(prevHash, this.pendingTransactions, nonce.toString());
        }
        let final_hash = hash
